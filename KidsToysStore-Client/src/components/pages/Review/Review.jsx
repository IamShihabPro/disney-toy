...
const Review = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch("https://disney-toys-server.vercel.app/reviews", {
      method: "POST",
  ...
  return (
    <div className="pt-10">
      <h3 className={`text-2xl mb-3 text-center font-semibold mt-4 px-2 `}>
        Review
      </h3>
      <form
            ...
            return (
              <label key={i} className="flex">
                <input
                  ...
                  className="hidden"
                  value={currentReview}
                  onClick={() => setReview(currentReview)}
                />
                <FaStar
                  className="mx-1 cursor-pointer "
                  size={30}
                  color={
                    currentReview <= (hover || review) ? "#ffc107" : "#e4e5e9"
                  }
